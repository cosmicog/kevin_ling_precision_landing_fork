...
extern cv::Vec4f __distParam; // Declared in ros_april.cpp
...
Eigen::Matrix4d TagDetection::getRelativeTransform(double tag_size, double fx, double fy, double px, double py) const {
  ...
  cv::Matx33f cameraMatrix(
                           ...
                           0, fy, py,
                           0,  0,  1);
  cv::solvePnP(objPts, imgPts, cameraMatrix, __distParam, rvec, tvec);
  cv::Matx33d r;
  cv::Rodrigues(rvec, r);
  Eigen::Matrix3d wRo;
...
//  ROS_INFO("Tag translation: %f, %f, %f", tvec.at<double>(0), tvec.at<double>(1), tvec.at<double>(2));
