...
std::vector<TagDetection> TagDetector::extractTagsOriginal(const cv::Mat& image) 
{
#ifdef DEBUG_TIME_PROFILE
  static int count = 0;
...
#ifdef DEBUG_TIME_PROFILE
  static double conversion_time_accumulator = 0;
  conversion_time_accumulator += (ros::Time::now() - start_time).toSec();
  if (count == 99) {
    ROS_INFO("conversion time: %f", conversion_time_accumulator / 100.0);
    conversion_time_accumulator = 0;
  }
#endif
...
#ifdef DEBUG_APRIL
  {
    for (unsigned int qi = 0; qi < quads.size(); qi++ ) {
      ...
      cv::line(cv_image, cv::Point2f(p1.first, p1.second), cv::Point2f(p2.first, p2.second), cv::Scalar(0,0,255,0) );
      cv::line(cv_image, cv::Point2f(p2.first, p2.second), cv::Point2f(p3.first, p3.second), cv::Scalar(0,0,255,0) );
      cv::line(cv_image, cv::Point2f(p3.first, p3.second), cv::Point2f(p4.first, p4.second), cv::Scalar(0,0,255,0) );
      cv::line(cv_image, cv::Point2f(p4.first, p4.second), cv::Point2f(p1.first, p1.second), cv::Scalar(0,0,255,0) );

      p1 = quad.interpolate(-1,-1);
      p2 = quad.interpolate(-1,1);
      p3 = quad.interpolate(1,1);
      p4 = quad.interpolate(1,-1);
      cv::circle(cv_image, cv::Point2f(p1.first, p1.second), 3, cv::Scalar(0,255,0,0), 1);
      cv::circle(cv_image, cv::Point2f(p2.first, p2.second), 3, cv::Scalar(0,255,0,0), 1);
      cv::circle(cv_image, cv::Point2f(p3.first, p3.second), 3, cv::Scalar(0,255,0,0), 1);
      cv::circle(cv_image, cv::Point2f(p4.first, p4.second), 3, cv::Scalar(0,255,0,0), 1);
    }
    cv::imshow("debug_april", cv_image);
  }
#endif
  ...
  for (unsigned int qi = 0; qi < quads.size(); qi++ ) {
	...
	if ( v > threshold)
	  tagCode |= 1;
        ...
        {
          if (v>threshold)
            cv::circle(cv_image, cv::Point2f(irx, iry), 1, cv::Scalar(0,0,255,0), 2);
          else
            cv::circle(cv_image, cv::Point2f(irx, iry), 1, cv::Scalar(0,255,0,0), 2);
        }
#endif
      }
    }
...
#ifdef DEBUG_TIME_PROFILE
  static double total_time_accumulator = 0;
  total_time_accumulator += (ros::Time::now() - start_time).toSec();
  if (count == 99) {
    ROS_INFO("total time: %f", total_time_accumulator / 100.0);
    total_time_accumulator = 0;
    count = 0;
  } else {
    count++;
  }
...
// roi is a pointer to the region of interest cv::Mat, or NULL if there is no windowing
//
// The strategy for windowing is to perform quadrilateral detection in the windowed area only
// and offset the found quadrilateral corners to the full image frame. The rest of the tag
// detection is done on the full image since that should all be fairly cheap.
std::vector<TagDetection> TagDetector::extractTags(const cv::Mat& image_, cv::Mat* roi, int offset_x, int offset_y) 
{
  int width = image_.cols;
  int height = image_.rows;
  std::pair<int,int> opticalCenter(width/2, height/2);

  // is the image windowed?
  ...
  if (using_roi)
  {
    cv::Canny((cv::Mat&) *roi, cannyEdges, 100, 300, 3);
  }
  else
  {
    cv::Canny(image_, cannyEdges, 100, 300, 3);
  }

#ifdef DEBUG_APRIL
  imshow("Canny", cannyEdges); // OpenCV call
  cv::waitKey(30);
#endif 


  //Get lines some how (findContours or Hough Transform)
  std::vector<std::vector<cv::Point> > contours;
  cv::findContours(cannyEdges.clone(), contours, CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE);
  ...
  for (int i = 0; i < (int)contours.size(); i++)
  {
    ...
    {
      //add to quads
      vector< std::pair<float,float> > corners;
      for(int j = 0; j < 4; j++)
      {
        float px, py;
        ...
        if (using_roi)
        {
          px = approx[j].x + offset_x;
          py = approx[j].y + offset_y;
        }
        else
        {
          px = approx[j].x;
        ...
        corners.push_back(std::pair<float,float>(px, py));
      }
      Quad quad = Quad(corners,opticalCenter);
  ...
  for (unsigned int qi = 0; qi < quads.size(); qi++ ) {
    ...
    for (int iy = -1; iy <= dd; iy++) {
      ...
      for (int ix = -1; ix <= dd; ix++) {
        ...
        if (irx < 0 || irx >= width || iry < 0 || iry >= height)
          continue;
        float v = image_.at<uchar>(iry, irx) / 255.0;
        if (iy == -1 || iy == dd || ix == -1 || ix == dd)
          whiteModel.addObservation(x, y, v);
        else if (iy == 0 || iy == (dd-1) || ix == 0 || ix == (dd-1))
          blackModel.addObservation(x, y, v);
...
#ifdef DEBUG_APRIL
  {
    ...
    for (unsigned int qi = 0; qi < quads.size(); qi++ ) {
      ...
      std::pair<float, float> p3 = quad.quadPoints[2];
      std::pair<float, float> p4 = quad.quadPoints[3];
      cv::line((cv::Mat&)image_, cv::Point2f(p1.first, p1.second), cv::Point2f(p2.first, p2.second), cv::Scalar(0,0,255,0) );
      cv::line((cv::Mat&)image_, cv::Point2f(p2.first, p2.second), cv::Point2f(p3.first, p3.second), cv::Scalar(0,0,255,0) );
      cv::line((cv::Mat&)image_, cv::Point2f(p3.first, p3.second), cv::Point2f(p4.first, p4.second), cv::Scalar(0,0,255,0) );
      cv::line((cv::Mat&)image_, cv::Point2f(p4.first, p4.second), cv::Point2f(p1.first, p1.second), cv::Scalar(0,0,255,0) );

      p1 = quad.interpolate(-1,-1);
      p2 = quad.interpolate(-1,1);
      p3 = quad.interpolate(1,1);
      p4 = quad.interpolate(1,-1);
      cv::circle((cv::Mat&)image_, cv::Point2f(p1.first, p1.second), 3, cv::Scalar(0,255,0,0), 1);
      cv::circle((cv::Mat&)image_, cv::Point2f(p2.first, p2.second), 3, cv::Scalar(0,255,0,0), 1);
      cv::circle((cv::Mat&)image_, cv::Point2f(p3.first, p3.second), 3, cv::Scalar(0,255,0,0), 1);
      cv::circle((cv::Mat&)image_, cv::Point2f(p4.first, p4.second), 3, cv::Scalar(0,255,0,0), 1);
    }
    
    cv::namedWindow("debug_april", CV_WINDOW_AUTOSIZE);
    cv::imshow("debug_april", image_);
    cv::waitKey(30);
