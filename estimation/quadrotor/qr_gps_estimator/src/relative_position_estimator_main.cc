...
struct GlobalEstimator
{
  ...
  geometry_msgs::Point tag_window_msg;
...
#ifdef DEBUG_MSGS
  g_est.corrected_attitudes.x = qr_cross_frame_rpy(0);
  g_est.corrected_attitudes.y = qr_cross_frame_rpy(1);
  g_est.corrected_attitudes.z = 0; // Who cares
...
void april_tag_cb(const geometry_msgs::Pose& msg) {
  ...
  Eigen::Vector3d meas_tag(msg.position.x, msg.position.y,
                           msg.position.z);
  ...
  g_est.tag_attitudes.x = wave_utils::radToDeg(g_est.tag_roll);
  g_est.tag_attitudes.y = wave_utils::radToDeg(g_est.tag_pitch);
  g_est.tag_attitudes.z = wave_utils::radToDeg(g_est.tag_yaw);
...
void reproject_tag_pos_to_camera_and_publish() {
  ...
  g_est.tag_window_msg.x = pred_tag_in_cf(0);
  g_est.tag_window_msg.y = pred_tag_in_cf(1);
  g_est.tag_window_msg.z = pred_tag_in_cf(2);
...
void publishAndMonitorTimerCallback(const ros::TimerEvent&) {
  ...
  height_kf.getStateEstimate(g_est.alt_kf_msg.x, g_est.alt_kf_msg.y,
                             g_est.alt_kf_msg.z);
  g_est.alt_kf_pub.publish(g_est.alt_kf_msg);

  g_est.qr_height = g_est.alt_kf_msg.x;

  g_est.tag_est_msg.z = g_est.qr_height;
  g_est.tag_est_msg.dz = g_est.alt_kf_msg.y;

  g_est.tag_est_msg.roll = g_est.corrected_attitudes.x;
  g_est.tag_est_msg.pitch = g_est.corrected_attitudes.y;
...
int main(int argc, char** argv) {
  ...
  g_est.tag_window_pub =
        nh.advertise<geometry_msgs::Point>("/wave/tag_window", 1);
  ...
  g_est.rotated_tag_pub =
        nh.advertise<geometry_msgs::Pose>("/wave/rotated_tag", 1);
//  ros::Subscriber ips_sub = nh.subscribe("/Robot_1/pose", 1, ipsCallback);
g_est.tag_attitudes_pub =
        nh.advertise<geometry_msgs::Point>("/wave/tag_attitudes",1);
  g_est.corrected_attitudes_pub =
        nh.advertise<geometry_msgs::Point>("/wave/corrected_att",1);
#endif

  g_est.alt_kf_pub = nh.advertise<geometry_msgs::Point>("/wave/alt_kf", 1);
